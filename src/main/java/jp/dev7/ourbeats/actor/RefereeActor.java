...
public class RefereeActor {
    ...
    public RefereeActor(NodeActor node0, NodeActor node1) {
        this.node0 = node0;
        this.node1 = node1;
    }
    ...
    @OnSpawn
    public void onSpawn() {
        on(topology).send(new ConnectedMessage(address, this, node1, 0)).to(
                node0.getAddress());
        on(topology).send(new ConnectedMessage(address, this, node0, 1)).to(
                node1.getAddress());
    }

    synchronized public void receive(NodeActor from, String data)
            throws IOException {
        ...
        if (receiveData0 != null && receiveData1 != null) {
            ...
            map.put("inputs",
                    new Object[] { JSON.decode(receiveData0),
                            JSON.decode(receiveData1) });
